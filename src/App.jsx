...
// ââ Helper: convert DB row â app recipe object ââ
...
// ââ Helper: convert app recipe â DB row for insert/update ââ
...
<p style="color:#888;font-size:12px;">\${recipes.length} recipes total, \${recipesToExport.length} with full details. Exported \${new Date().toLocaleDateString()}</p>`;
  ...
  // ââ Check URL for shared recipe token on mount ââ
  useEffect(() => {
  ...
  // ââ Load recipes from Supabase on mount ââ
  useEffect(() => {
    if (!currentUser) {
  ...
  if (loading && currentUser) {
    return (
      <div style={{ minHeight:"100vh", background:"#f7f6f3", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <Spinner label="Loading recipesâ¦" />
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f7f6f3", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      {/* ââ SHARED RECIPE VIEW (accessed via ?share=token) ââ */}
            ...
            <div style={{ background:"#fff", borderRadius:20, overflow:"hidden", boxShadow:"0 2px 16px rgba(0,0,0,0.08)" }}>
              ...
              <div style={{ padding:"20px 20px 28px" }}>
                ...
                <div style={{ display:"flex", gap:14, marginTop:10, fontSize:12, color:"#888" }}>
                  {sharedRecipe.time && <span>â± {sharedRecipe.time}</span>}
                  {sharedRecipe.servings && <span>ð¤ {sharedRecipe.servings}</span>}
                </div>
          ...
          <div style={{ background:"#062846", padding:"0 16px", position:"sticky", top:0, zIndex:100, boxShadow:"0 2px 12px rgba(0,0,0,0.18)" }}>
            <div style={{ maxWidth:720, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:52 }}>
              ...
              <nav style={{ display:"flex", gap:1, alignItems:"center" }}>
                {[{label:"Recipes",v:views.HOME},{label:"Favs",v:views.FAVORITES},{label:"+ Add",v:views.ADD},{label:"ð Log",v:views.LOG}].map(n => (
                      ...
                      {favs.map(r => (
                        <div key={r.id} onClick={() => { setSelected(r); setView(views.DETAIL); }}
                          ...
                          <div style={{ display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
                            <StarRating value={r.rating||0} />
                            <span onClick={e => { e.stopPropagation(); toggleFav(r.id); }}
                              style={{ fontSize:16, cursor:"pointer", color:r.favorite?"#ff492c":"#ccc", transition:"color 0.15s" }}>â¥</span>
                          </div>
                        </div>
                      ))}
                    </div>
...
{true && (
                  <div style={{ background:"#fff", borderRadius:18, padding:20, boxShadow:"0 2px 12px rgba(0,0,0,0.07)" }}>
                    ...
                    <div style={{ marginBottom:18 }}>
                      ...
                      <textarea value={manualNotes} onChange={e=>setManualNotes(e.target.value)} placeholder="Any tips, variations, or remindersâ¦"
                    ...
                    <button onClick={async () => {
                      ...
                      {manualSaving ? "Savingâ¦" : "Save Recipe"}
                    </button>
                  </div>
                )}
              </div>
                  ...
                  <div style={{ padding:"20px 20px 28px" }}>
                    ...
                    {editMode ? (
                      <div style={{ marginTop:16 }}>
                        ...
                        <input value={editUrl} onChange={e => setEditUrl(e.target.value)} placeholder="Paste recipe URL..." style={{ width:"100%", padding:"8px 10px", borderRadius:8, border:"1.5px solid #ddd", fontSize:12, fontFamily:"'DM Sans',sans-serif" }} />
                      </div>
                    ) : selected.url ? (
                      <div style={{ marginTop:16 }}>
